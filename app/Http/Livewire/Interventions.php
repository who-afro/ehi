...
class Interventions extends Component
{
    ...
    public function getInterventionList(): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        return Intervention::with('condition:id,name', 'ageCohort:id,name', 'interventionLevel:id,name', 'publicHealthFunction:id,name', 'programAreas', 'interventionGroups')
            ...
            ->when($this->filters['program_area_id'], fn($query, $program_area_id) => $query->whereHas('programAreas', function($query) use ($program_area_id) {
                $query->where('program_area_id', $program_area_id);
            }))
            ->when($this->filters['intervention_group_id'], fn($query, $intervention_group_id) => $query->whereHas('interventionGroups', function($query) use ($intervention_group_id) {
                $query->where('intervention_group_id', $intervention_group_id);
            }))
            ->paginate(10);
    }
